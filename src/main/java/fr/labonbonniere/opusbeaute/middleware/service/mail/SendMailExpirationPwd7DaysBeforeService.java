 ...
 */
@Stateless
public class SendMailExpirationPwd7DaysBeforeService {
	
	// Specifie le nombre de jours avant expiration.
	static Integer nbJrs = 7;
	...
	static final Logger logger = LogManager.getLogger(SendMailExpirationPwd7DaysBeforeService.class.getSimpleName());
	...
	public void envoyerMailUtilisateurPwdExpireDansXJours() throws Exception {
		...
		logger.info("SendMailExpirationPwd7DaysBeforeService log : Nombre d objets Utilisateurs : " + nbUtilisateurs);
		...
		for (int nbObj = 0;  nbObj <= nbUtilisateurs-1; nbObj++) {			
			logger.info("SendMailExpirationPwd7DaysBeforeService log : Position dans la liste d objets Utilisateur : " + nbObj);	
	...
	private List<Utilisateur> recupereListeUtilisateurs() throws Exception {
		...
		try {
			...
			logger.info("SendMailExpirationPwd7DaysBeforeService log : listeUtilisateurs  : " + listUsers.toString());
		} catch (DaoException message) {
			throw new Exception("SendMailExpirationPwd7DaysBeforeService Exception : "
	...
	private Date plususXDaysFromLaDateDuJourZDT() {
		...
		logger.info("SendMailExpirationPwd7DaysBeforeService log : ZDT to Date -" + nbJrs + " jours : " + dateminXDays);
	...
	private Date convertTimestampToDate(Timestamp dateTsPwdExpiration) {
		...
		logger.info("SendMailExpirationPwd7DaysBeforeService log : Ts to Date : " + datePwdExp);
	...
	private Timestamp checkIfPwdExpirationDateIsNull(Utilisateur userPwdToCheck) {
		...
		logger.info("SendMailExpirationPwd7DaysBeforeService log : Recuperation de la date ");
		if (userPwdToCheck.getPwdExpirationDateTime() == null) {
			logger.info("SendMailExpirationPwd7DaysBeforeService log : La date est nulle");
	...
	private boolean isDatePwdMatchingWithDateMinusXDays(Date dateJJPlusXdays, Date datePwdExp) {
		...
		logger.info("SendMailExpirationPwd7DaysBeforeService log : val de datminus : " + dateJJPlusXdays);
		logger.info("SendMailExpirationPwd7DaysBeforeService log : val de pwdExpire: " + datePwdExp);
		
		if (DateUtils.isSameDay(dateJJPlusXdays, datePwdExp) == true) {
			logger.info("SendMailExpirationPwd7DaysBeforeService log : la date du pwd est encore valide");
			return true;
		} else {
			logger.info("SendMailExpirationPwd7DaysBeforeService log : la date du pwd n est plus valide valide");
			return false;
		}		
