...
public class SendSmsReminderClientService {
	...
	public void sendSmsClientTriggerWsTest() throws Exception {
...
//			logger.info("MailRemiderSender log : SUSreminder : " + isSucribedSmsReminder);
...
//				logger.info("MailRemiderSender log : Conversion TS to Date : " + rdvHeure);
				...
				String messageCorps01 = "La%20Bonbonnière%20d%27%20Audrey,%0d%0avous%20rappèle%20votre%20rendez-vous%0d%0ade%20demain%20à%20" + rdvHeure + ".";
				String messagePrestaSoin = "%0d%0aPour%20le%20soin%20:%0d%0a" + activite.toUpperCase() + "_" 
									+ WordUtils.capitalizeFully(prestationsoin).replaceAll(" " , "%20").replace("è", "%C3%A8").replace("é", "%C3%A9") + "%0d%0a" + "%0d%0a";
...
//				messageAdressePart1 = messageAdressePart1.replace(" ", "%20");
				String messSms = messageClientPrenom.replace(" ", "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")
						+ messageCorps01.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")
						+ messagePrestaSoin.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")
						+ messageAnnonceAdresse.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")
						+ messageLieuRdv.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")						
						+ messageAdressePart1.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9").replace("À", "%C3%80")
						+ messageAdressePart2.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9")
						+ messageFinPart1
						+ messageFinPart2.replaceAll(" " , "%20").replace("à", "%C3%A0").replace("è", "%C3%A8").replace("é", "%C3%A9");
				// URL url = new URL("http://192.168.1.50/RaspiSMS/smsAPI/?email=admin@example.fr&password=admin" + msg);
				String buildUrl ="http://" + adresseIp + "/RaspiSMS/smsAPI/?email=" 
								+ raspiSmsLogin + "&password=" + raspiSmsPwd + "&numbers=" + clientTelMobile + "&text=" + messSms;	
